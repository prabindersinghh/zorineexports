# Zorine Exports — Tech Stack

Premium Indian agricultural export company website built to Awwwards/luxury brand standards.

**Live:** [zorineexports.com](https://zorineexports.com) · **Repo:** [github.com/prabindersinghh/zorineexports](https://github.com/prabindersinghh/zorineexports)
...
### Animation Strategy
- **Above-fold content** uses `animate` (fires on mount) via `HeroReveal` component — ensures hero text always appears on SSR hydration
- **Below-fold content** uses `whileInView` (fires on scroll) via `Reveal` component with directional variants (`up`, `left`, `right`)

### Button Color System
Explicit `.btn-primary`, `.btn-cream`, `.btn-outline`, `.btn-ghost` CSS classes with `color: var(--cream) !important` override the global `a { color: inherit }` cascade — ensuring correct text colors on all link-buttons regardless of context
...
### Ticker Strip
Dark green horizontal marquee between nav and hero — pure CSS `@keyframes` infinite scroll with no JavaScript overhead, pauses on hover
